...
#define MYUDPPORT "22337"  // the udp port for the connection with backend servers
...
#define OPERATORDIST 4 // size of 'and '
...
void calculate(char *line, char **&results, int &curr_row_index) {
    ...
    results[curr_row_index][results_index++] = 'a';
    results[curr_row_index][results_index++] = 'n';
    results[curr_row_index][results_index++] = 'd';
    results[curr_row_index][results_index++] = ' ';
...
{
    // the following piece is referred from beej (line 111 to 148)
    ...
    printf("The Server AND start receiving lines from the edge server for OR computation.\nThe computation results are:\n");
